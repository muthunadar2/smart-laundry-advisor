...
Do not include markdown or extra text."""
    ...
    data["load_kg"] = max(
        0.0, min(10.0, float(data["load_kg"]))
    )

    data["dirt_level"] = max(
        0, min(100, int(data["dirt_level"]))
    )

    data["water_saving"] = max(
        0, min(100, int(data["water_saving"]))
    )
...
Use the supplied result and do not change it.

Explain the recommendation in 3-5 simple sentences.
